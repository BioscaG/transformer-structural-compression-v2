...
# ── Page palette (in sync with web/index.html) ──────────────────────────
PAGE_BG       = "#F7F6F2"
PAGE_BG_PLOT  = "#FFFFFF"
PAGE_INK      = "#141413"
PAGE_INK_2    = "#4A4A47"
...
FONT_STACK = ('"Geist", "Inter", -apple-system, BlinkMacSystemFont, '
              '"Helvetica Neue", sans-serif')
FONT_MONO  = '"Geist Mono", "JetBrains Mono", "SF Mono", Menlo, monospace'

# Web-native sizes (originals — NOT thesis-print sizes).
SZ_TITLE      = 13
SZ_LABEL      = 11
SZ_TICK       = 10
SZ_LEGEND     = 10
...
# Heights are author-declared in web/sections.py (FIG_HEIGHTS).
...
def apply_site_mode(fig: go.Figure) -> go.Figure:
    ...
    fig.update_layout(
        ...
        margin=dict(l=92, r=32, t=28, b=56),
        autosize=True,
        width=None,
        hoverlabel=dict(
    ...
    # Annotations cleanup:
    #   - Subplot titles → DROPPED (the section header outside the iframe
    #     handles labelling; we want zero titles inside the figure).
    #   - Top-of-paper title-like annotations (y ≥ 0.98) → DROPPED.
    ...
    def _clean_annotations(anns):
        ...
        for ann in anns:
            ...
            if yref.endswith("paper") and y is not None and y < -0.04 and has_bg:
                continue
            # Drop subplot titles auto-generated by make_subplots
            is_subplot_title = (
                xref.endswith("paper") and yref.endswith("paper")
                and d.get("yanchor") == "bottom"
            )
            if is_subplot_title:
                continue
            # Drop title-like annotations placed above the chart in paper
            # coords (custom subplot title shims)
            is_top_title_shim = (
                xref.endswith("paper") and yref.endswith("paper")
                and y is not None and y >= 0.98
                and not has_bg
            )
            if is_top_title_shim:
                continue
            out.append(d)
        return out
    ...
    fig.update_xaxes(
        ...
        tickfont=dict(family=FONT_MONO, size=SZ_TICK, color=PAGE_INK_3),
    ...
    if fig.layout.sliders:
        ...
        for sl in fig.layout.sliders:
            ...
            d["ticklen"]         = 4
            d["minorticklen"]    = 0
            d["font"] = dict(family=FONT_MONO, size=10.5,
                             color=PAGE_INK_3)
