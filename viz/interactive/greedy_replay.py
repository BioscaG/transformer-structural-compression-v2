...
def build_replay_figure() -> go.Figure:
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Matriz de compresión · 12 capas × 6 componentes",
                        "Acumulado a lo largo del recorrido"),
    ...
    fig.add_trace(go.Heatmap(
        ...
        colorbar=dict(thickness=14, len=0.8, x=0.55,
                      title=dict(text="rango log<sub>2</sub><br>(rojo=más compresión)",
                                 font=dict(size=11)),
                      tickmode="array",
    ...
    frames = []
    captions = {
        "baseline": "Modelo base, ningún componente comprimido. F1 = 0.469, "
                    "ratio = 1.00× (todo a rango 768).",
        "greedy_95": "Greedy 95% — 24 movimientos baratos. Solo Q y K en algunas "
                     "capas. Casi sin coste de F1: el algoritmo está aprovechando "
                     "que Q y K son inmunes a compresión agresiva (§4.3).",
        "greedy_90": "Greedy 90% — 28 movimientos. El algoritmo añade FFN-output "
                     "en capas tempranas. Sigue siendo prácticamente gratis. "
                     "La banda right side del grid empieza a iluminarse.",
        "greedy_85": "Greedy 85% — 32 movimientos. Más V y FFN-output. F1 sigue "
                     "alto, ratio bajando. Aún sin tocar capas tardías intactas.",
        "greedy_80": "Greedy 80% — 36 movimientos. Llega al primer FFN-intermediate. "
                     "Empieza a notarse la pérdida de F1, pero todavía controlada.",
        "greedy_75": "Greedy 75% — 40 movimientos. La parte FFN del grid se llena. "
                     "El algoritmo evita SISTEMÁTICAMENTE las capas 8-11.",
        "greedy_70": "Greedy 70% — 52 movimientos. El algoritmo entra en zona "
                     "agresiva. Las capas tardías (8-11) siguen relativamente "
                     "intactas — el cuello de botella se respeta.",
        "greedy_60": "Greedy 60% — 64 movimientos. Casi todo comprimido excepto las "
                     "capas tardías clave. F1 cae notablemente.",
        "greedy_50": "Greedy 50% — 64 movimientos. Compresión máxima del greedy. "
                     "Las capas tardías de FFN-int sobreviven hasta el final. "
                     "F1 cerca del colapso pero ratio drástico.",
    }
    ...
    fig.update_layout(
        **st.thesis_layout(
        ...
        annotations=[
            dict(text="Matriz de compresión · 12 capas × 6 componentes",
                 x=0.31, y=1.04, xref="paper", yref="paper", showarrow=False,
                 font=dict(size=12.5, color=st.INK_2, family="serif")),
            dict(text="Trayectoria F1 vs ratio",
                 x=0.85, y=1.04, xref="paper", yref="paper", showarrow=False,
                 font=dict(size=12.5, color=st.INK_2, family="serif")),
    ...
    fig.update_yaxes(title=dict(text="ratio / retention", font=dict(size=11, color=st.INK_2)),
